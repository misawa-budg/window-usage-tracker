...
- 一覧タイムラインを表示できること
  - `24h`: `Active / Open / Minimized` の3レーンを00:00-24:00の横棒で表示
  - `1week`: 日付ごとに1行、各行を00:00-24:00スケールで表示（1週間分）
  - 凡例として上位アプリ色を表示する（上限あり、超過は `Other`）
- アプリ別タイムラインを表示できること
  - アプリ選択で対象を絞れること
  - `24h`: 選択アプリの `Active / Open / Minimized` 3レーン表示
  - `1week`: 選択アプリの日付行表示（各日00:00-24:00スケール）
  - 凡例として状態色（`Active/Open/Minimized`）を表示すること
- 行ごとの `Total`（HH:mm）を表示できること

## 集計ルール
- 区間重なりで利用秒数を算出する（期間をまたぐイベントを正しく計算する）
- 基本集計バケットは1時間（`BucketSize = 1h`）
- `24h` は「当日00:00〜翌日00:00（ローカル）」を対象にする
- `1week` は「当日を含む直近7日（ローカル日境界）」を対象にする
- 日次行の表示は24時間幅を100%として描画し、行内の秒数は1時間上限で正規化する
