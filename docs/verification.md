...
## v0.3.2 表示処理の整理（2026-09-20）

- `TimelineDisplayData` で表示区間を再利用し、`TimelineViewModelFactory` へWinUI変換、`TimelinePresentation` へ色・時刻整形を分離。Collectorの収集・保存・通信規則、DBスキーマ、XAMLの見た目は変更していない。
- 合成2,000区間と空／1区間 × 日／週 × 3モードの18条件で、整理前後の描画モデルJSONが完全一致（SHA-256: `080084F88B00D186AE0111274CFF15B58D817413EA1493C9CE3B0A62D7A1F532`）。同一PC・カルチャ・タイムゾーンでの比較であり、実画面描画や全環境の保証ではない。
- 表示モードの合計・再利用・リセット、時刻丸め・24時・色の規則、JavaScript/C#共通の32分類ケースを追加。Collector 50件、Viewer/Shared 103件、拡張53件の計206件。Computer Useと画面起動は行わず、目視・別PC・多数サイトの性能・長期負荷は今後の確認対象。

## v0.3.1 ホスト名の任意記録（2026-09-20）

- 未対応サイトを列挙し続ける代わりに、明示的な `storeBrowserHostnames` 設定時だけ正規化したホスト名を記録・表示する。配布設定ではfalse。既存の `service_id` と前面区間の置換を使い、0.3.0からのスキーマ追加・既存行の書き換えはない。設定を戻しても過去の記録は表示する。
- Collector 50件、Viewer/Shared 60件、Node 53件の計163件成功。追加項目は設定既定値・読込、照会ごとの同意フラグ、厳密なboolean判定、保存層での拒否、ホスト名の形式・長さ、IP・パス・クエリ等の排除、サブドメインの分離、前面合計の維持。新規NuGet/npm依存はない。
- `dotnet list WinTracker.slnx package --vulnerable --include-transitive` は全6プロジェクトで既知脆弱性の指摘なし。利用可能なNuGet情報の検査であり、OS・全ランタイム・未知の欠陥の保証ではない。
- 配布検査を拡張し、全設定ファイルでブラウザ連携・ホスト名保存・タイトル保存が明示的にfalseであること、個人用ホスト登録JSONがないこと、Portable内のCollector・Viewer・BrowserHost・拡張の版が揃うことを確認する。従来のDB・ログ混入検査は維持。
- 直前の0.3.0実機確認では、通常起動元での登録後、Edge拡張のCONNECTEDと実DBのサービス記録を確認した。開発側で見える登録だけでは通常Edgeの接続を保証できなかった。実行環境によるレジストリ可視性の違いは確認したが、内部原因は断定しない。
- 今回Computer Use・ブラウザの自動操作・Viewer起動は行わない。0.3.1実画面の目視、Chrome実機、複数プロフィール、長期負荷、多数ホスト時の性能、クリーンOSは別途確認が必要。アプリ単位の過去の性能値を、多数サイト表示でも同じと主張しない。

以下は各時点の検証履歴。過去の「未実施」はその時点の記録であり、現在の確認範囲は上記およびリリースノートを参照する。
...
## 時間ラベルの補助線合わせ（2026-09-20）

- 06:00・12:00・18:00の左端が四分点に置かれていたため、ラベルの中心が右へずれて見えていた。共通TimeAxisTemplateの内部ラベルを隣接2列にまたがる中央配置へ変更し、文字の中心を6・12・18時の補助線位置へ合わせた。固定ピクセルの補正、コードビハインド、データ・集計処理の変更はない。00:00と24:00は表示領域の端で見切れないよう従来の端揃えを維持。
- 共通テンプレートは日／週表示の上下両軸に使われる。既存テストを更新し、幅375.5・720・1086で内部ラベルの中央座標が補助線の座標と一致するXAML契約テストを3ケース追加。Viewerテスト41件成功、Release/self-containedビルドは警告0・エラー0。
- ユーザー指示によりComputer Use・画面起動・画面切替は実施しない。確認用は `artifacts/time-axis-centered/Run-Viewer-With-Desktop-Data.cmd`。修正版Viewerだけを起動しDesktop DBを読み取り専用で参照する。実画面の目視確認はユーザーが行う。Desktop配布物と公開済みv0.2.0は変更していない。
